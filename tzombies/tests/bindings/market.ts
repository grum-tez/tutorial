...
export class order_value implements att.ArchetypeType {
    constructor(public fa2: att.Address, public seller: att.Address, public token_id: att.Nat, public amount: att.Nat, public price: att.Tez, public expiry: Date) { }
    toString(): string {
        return JSON.stringify(this, null, 2);
    }
    to_mich(): att.Micheline {
        return att.pair_to_mich([this.fa2.to_mich(), this.seller.to_mich(), this.token_id.to_mich(), this.amount.to_mich(), this.price.to_mich(), att.date_to_mich(this.expiry)]);
    }
    equals(v: order_value): boolean {
        return att.micheline_equals(this.to_mich(), v.to_mich());
    }
    static from_mich(input: att.Micheline): order_value {
        return new order_value(att.Address.from_mich((input as att.Mpair).args[0]), att.Address.from_mich((input as att.Mpair).args[1]), att.Nat.from_mich((input as att.Mpair).args[2]), att.Nat.from_mich((input as att.Mpair).args[3]), att.Tez.from_mich((input as att.Mpair).args[4]), att.mich_to_date((input as att.Mpair).args[5]));
    }
}
...
export class Zombie_market {
    ...
    async deploy(params: Partial<ex.Parameters>) {
        const address = (await ex.deploy("./contracts/zombie_market.arl", {}, params)).address;
        this.address = address;
    }
